...
function remove (arr, bookName) {
  ...
  if (book_index >= 0) {
    let book2 = [...arr]
    book2.splice(book2.indexOf(bookName), 1);
    return book2;
...
var ratings = watchList.map(f =>{return {title: f.Title, rating: f.imdbRating}} )
...
var filteredList = watchList.filter(function(x){
  let rating = Number(x.imdbRating);
  if (rating >= 8){
    return rating
  }
  }).map(function(x) {
      var title = x.Title;
      var rating = x.imdbRating;
      var obj = {
        title,
        rating
      };
